...
describe("compose()", function () {
  ...
  it("throws an error when an array composed only of middleware functions is not provided", function () {
    var e;
    try {
      var test = compose([mw, "not a func", mw]);
    } catch (err) {
      e = err;
    }
    assert.instanceOf(e, TypeError);
  ...
  it("returns a function that handle asynchronous operations via promises", function () {
    ...
    var next = sinon.spy(() => {})
    var test = compose([waitMw, waitMw]);
  ...
  it("can be composed with other middleware as nested middlewares", function () {
    var test1 = sinon.spy(compose([mw]));
    var test2 = sinon.spy(compose([mw, mw]));
    var test3 = compose([mw, test1, test2]);
